@includeWhen(!count($userInfo->posts), 'alerts.empty', ['msg' => 'ليس هناك منشورات لعرضها'])
              ...
              <div class="row">
                  <div class="col-12">
                    @if (Auth::check())
                      ...
                      <form action="{{ route('post.destroy' , $post->id) }}" method="POST" onsubmit="return confirm('هل تريد حذف المنشور')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="float-left"><i class="far fa-trash-alt text-danger fa-lg ml-2"></i></button>
                      </form>
                      @endcan 
                    @endif
